<template>
  ...
  <span >
    ...
    <blog-section v-else  style="min-width: 260px" title="目录">
      <a-anchor :affix="false" :targetOffset="60">

      <template   v-for="(item,index) in tags">
        <a-anchor-link :style="{marginLeft:(item.offset-1)*5+'px'}" v-if="item.a_id"  :href="'#'+item.a_id" :title="item.innerText" />
      </template>
